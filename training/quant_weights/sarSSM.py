...
class SSKernelDiag(nn.Module):
    ...
    def setup_step(self):
        ...
        # Incorporate dt into A
        dtA = w * dt.unsqueeze(-1)  # (H N)
        self.dA = nn.Parameter(torch.exp(dtA), requires_grad=False) # (H N)
        self.dC = nn.Parameter((C * (torch.exp(dtA)-1.) / w), requires_grad=False)# (C H N)
        self.dB = nn.Parameter(self.dC.new_ones(self.H, self.N), requires_grad=False) # (H N)
...
class S4D(nn.Module):
    ...
    def __init__(
            ...
            bidirectional=False,
            # Arguments for FF
            activation='leakyrelu', # activation in between SS and FF
