...
class CoarseRDA:
    ...
    def _get_reference_function(self, replica_chirp: np.ndarray, window_type: str = 'hamming') -> np.ndarray:
        ...
        # Calculate energy normalization (matching C++ implementation exactly)
        # C++: F_VEC_1D norm = magnitude_1d(replica_chirp);
        norm = np.abs(windowed_replica)
        norm_size = len(norm)
        
        # C++: std::for_each(norm.begin(), norm.end(), [norm_size](std::complex<double> n) { n *= n / norm_size; });
        # Note: The C++ code is incorrect - it treats norm as complex but it's real. The intent is norm *= norm / norm_size
        norm_modified = norm * norm / norm_size
        
        # C++: double energy = std::accumulate(norm.begin(), norm.end(), 0.0);
        ...
        # C++: compute_1d_dft_in_place(reference, 0, false); conjugate_in_place(reference);
        reference = np.fft.fft(reference)
        reference = np.conjugate(reference)
        
        # Normalize by energy
        # C++: std::for_each(reference.begin(), reference.end(), [energy](std::complex<double> &n) { n /= energy; });
        if energy > 0:
    ...
    def pulse_compression(self, signal: np.ndarray, reference: np.ndarray) -> np.ndarray:
        """Perform pulse compression following C++ implementation.
        
        Args:
            signal: Input signal array.
            reference: Reference function for compression.
            
        Returns:
            Pulse compressed signal.
        """
        # Store original number of samples (matching C++ implementation)
        num_samples = signal.shape[1] if len(signal.shape) > 1 else len(signal)
        
        # Element-wise multiplication (matching C++ implementation)
        # C++: std::transform(reference.begin(), reference.end(), signal_.begin(), signal_.begin(),
        #                    [] (const std::complex<double>& n, std::complex<double>& r) { return n * r;})
        compressed_signal = signal * reference
        
        # C++: return CF_VEC_1D(signal_.begin(), signal_.begin() + num_samples);
        # This just returns the signal without truncation since num_samples == signal.size()
    ...
    def get_range_filter(self, pad_w: int = 0, window_type: str = 'hamming') -> np.ndarray:
        ...
        Args:
            pad_w: Width padding for FFT length optimization.
        ...
        Raises:
            ...
            ValueError: If window type is not supported.
        """
        # Use exact radar data dimensions - no padding for now to match original behavior
